...
void handleSavePage( AsyncWebServerRequest *request )
{
  ...
  paramChars( request, appcfgWR.alexa_devicename, "alexa_devicename", DEFAULT_ALEXA_DEVICENAME );
...
void WebHandler::setup()
{
  ...
  server.on("/info", HTTP_GET, [](AsyncWebServerRequest *request) {
    AsyncWebServerResponse *response = request->beginResponse(200, "application/json", jsonInfo() );
    response->addHeader( "Access-Control-Allow-Origin", "*" );
    request->send(response);
  });
  ...
  server.on("/info.html", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    response->printf("<p><a href='http://%s/on'>http://%s/on</a> - Socket ON</p>"
      ...
      ipAddress, ipAddress, ipAddress, ipAddress, ipAddress, ipAddress, ipAddress, ipAddress );
